...
We utilized a spatial Cormack-Jolly-Seber (CJS) model \citep{} to account for seasonal detection probabilities. 
...
where $\sigma_0$ is the standard deviation describing the dairy distance moved between occasion $t$ and $t+1$ and $\eta_{j,t}$ is the interval (unit: day) between the occasions for individual $j$.
