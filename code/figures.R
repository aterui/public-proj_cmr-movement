...
all_movement <- gghistogram(df_m, x= "move", fill = "dodgerblue",
            xlab = "Distance (m)", ylab = "Frequency", binwidth = 10) +
  geom_vline(xintercept = 0, linetype="dashed", color = "red", size=0.9) +
...
# GAM visual of density vs section relationship
...
ggplot(df_m[df_m$species %in% c('bluehead_chub', 'striped_jumprock', 'creek_chub', 'bluegill', 'green_sunfish','redbreast_sunfish'), ], 
       aes(x = section_cap,
           y = density, 
           color= species)) +
  geom_point(alpha = 0.2) +
...
# GAM visual of movement vs section 
ggplot(df_m[df_m$species %in% c('bluehead_chub', 'striped_jumprock', 'creek_chub', 'bluegill', 'green_sunfish','redbreast_sunfish'), ], 
       aes(x = section_cap,
           y = density, 
           color= species)) +
